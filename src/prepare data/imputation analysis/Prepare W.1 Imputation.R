...
ge.vars <- c('h1gh21', 'h1da5', 'h1fv5', 'h1gh28', 'h1pr4', 'h1da10',
             'h1gh46', 'h1ee4', 'h1ed7', 'h1fs2', 'h1gh39', 'h1da11', 'h1da1',
             'h1pf15', 'h1pr1', 'h1gh20', 'h1pf32', 'h1id5', 'h1da6', 'h1pf16',
             'h1gh29', 'h1pf10', 'h1ee2', 'h1fs4', 'h1gh42')


####################################################################
# Determine extent of missingness
wave.1$na_count <- apply(is.na(wave.1[,ge.vars]), 1, sum)

# create a data frame with the ge.vars
ge.df <- wave.1[, c('bio_sex', ge.vars, 'na_count')]
colSums(is.na(filter(ge.df, bio_sex == 1)))

# count the number of missing values by row (ID)
missing <- rowSums(is.na(ge.df)) > 0
num_missing <- sum(missing)


wave.1 <- wave.1 %>% 
  group_by(bio_sex) %>% 
  mutate_at(ge.vars, ~ifelse(is.na(.), median(., na.rm = TRUE), .))
